...
export const metadata: Metadata = { title: 'SEBI LODR — Compliance' }

export default async function SebiPage() {
  const supabase = createClient()

  const { data: regime } = await supabase
    .from('compliance_regimes').select('id').eq('code', 'sebi_lodr').single()
  ...
  const [{ data: posture }, { data: items }, { data: flags }] = await Promise.all([
    ...
    supabase.from('contract_flags').select('id, contract_id, title, severity, is_resolved, contracts!contract_id(id, title)')
      .eq('category', 'sebi').eq('is_resolved', false).limit(20),
  ])

  return (
    <RegimePage
      regimeCode="sebi_lodr"
      regimeName="SEBI LODR"
      icon={TrendingUp}
